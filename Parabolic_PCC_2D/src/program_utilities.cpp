                                      ...
                                      const std::array<double, 2> &time_domain)
{
    ...
    switch (config.TimePartitionMethod())
    {
    case Program_Configuration::TimePartition::Uniform:
        return geometryUtilities.EquispaceCoordinates(config.NumTimeStep() + 1, time_domain[0], time_domain[1], true);
    case Program_Configuration::TimePartition::ExponentialIncreasing: {
        std::vector<double> times(config.NumTimeStep() + 1);
        const double alpha = -sqrt(config.NumTimeStep()) * 0.1;
        const double inv_N = 1.0 / config.NumTimeStep();
        for (unsigned int n = 0; n < config.NumTimeStep() + 1; n++)
        {
            times[n] = (time_domain[1] - time_domain[0]) * (exp(alpha * n * inv_N) - 1.0) / (exp(alpha) - 1.0) + time_domain[0];
            std::cout.precision(16);
            std::cout << std::scientific << times[n] << std::endl;
        }

        return times;
    }
    case Program_Configuration::TimePartition::ExponentialDecreasing: {
        std::vector<double> times(config.NumTimeStep() + 1);
        const double alpha = sqrt(config.NumTimeStep()) * 0.1;
        const double inv_N = 1.0 / config.NumTimeStep();
        for (unsigned int n = 0; n < config.NumTimeStep() + 1; n++)
                     ...
                     const std::string &exportVtuFolder)
{
...

    {
        ...
        std::cout << "ThetaValue" << separator;
        std::cout << "DeltaValue" << separator;
        std::cout << "TimeValue" << separator;
        ...
        std::cout << std::scientific << theta_value << separator;
        std::cout << std::scientific << delta_value << separator;
...

    {
        ...
        if (!errorFileExists)
        {
            ...
            errorFile << "ThetaValue" << separator;
            errorFile << "DeltaValue" << separator;
            errorFile << "TimeValue" << separator;
        ...
        errorFile << std::scientific << theta_value << separator;
        errorFile << std::scientific << delta_value << separator;
    ...
    if (config.PostProcess())
    {
        ...
        exporter.Export(exportVtuFolder + "/Solution_" + std::to_string(TEST_ID) + "_" + std::to_string(Method_ID) +
                        "_" + std::to_string(config.MethodOrder()) + "_" + std::to_string(time_id) + ".vtu");
    }
}
// ***************************************************************************
